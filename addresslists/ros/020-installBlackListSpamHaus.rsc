...
/system script add name="FetchBlacklistSpamHaus" source={
...
:if (:typeof[$AddressListsWebRemoteUser] = "nil") do={
    ...
    /tool fetch url="https://$AddressListsWebRemoteHost/spamhaus.rsc" mode=https;
    } else={
    # If a username was set assume authentication is mandatory and also use a password
    /tool fetch url="https://$AddressListsWebRemoteUser:$AddressListsWebRemotePassword@$AddressListsWebRemoteHost/spamhaus.rsc" mode=https;
}

}

/system script add name="ReplaceBlacklistSpamHaus" source={

# Declare list name including its extension (has to be .rsc)
:local listName "spamhaus.rsc";

# Declare comment used to identify all existing entries of this list
:local listComment "SpamHaus";
...
/system scheduler add interval=1d name="FetchBlacklistSpamHaus" on-event="/system script run FetchBlacklistSpamHaus" start-date=jan/01/1970 start-time=01:43:00
/system scheduler add interval=1d name="ReplaceBlacklistSpamHaus" on-event="/system script run ReplaceBlacklistSpamHaus" start-date=jan/01/1970 start-time=01:53:00

# Fetch and install address list
:put ">>> Fetching list ..."
/system script run FetchBlacklistSpamHaus;
:delay 5;
:put ">>> Installing entries ..."
/system script run ReplaceBlacklistSpamHaus;
