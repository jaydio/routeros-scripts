...
/system script add name="FetchBlacklistTorNodesV6" source={
...
:if (:typeof[$AddressListsWebRemoteUser] = "nil") do={
    ...
    /tool fetch url="https://$AddressListsWebRemoteHost/tornodes-v6.rsc" mode=https;
    } else={
    # If a username was set assume authentication is mandatory and also use a password
    /tool fetch url="https://$AddressListsWebRemoteUser:$AddressListsWebRemotePassword@$AddressListsWebRemoteHost/tornodes-v6.rsc" mode=https;
}

}

/system script add name="ReplaceBlacklistTorNodesV6" source={

# Declare list name including its extension (has to be .rsc)
:local listName "tornodes-v6.rsc";

# Declare comment used to identify all existing entries of this list
:local listComment "tornodes-v6";
	...
	:if ( [/file get [/file find name=$listName] size] > 1000 ) do={
        :log info "$listName: Removing existing entries";
			...
			# Identify all pre-existing entries and remove them
   	        :foreach entry in=[/ipv6 firewall address-list find] do={
...
/system scheduler add interval=1d name="FetchBlacklistTorNodesV6" on-event="/system script run FetchBlacklistTorNodesV6" start-date=jan/01/1970 start-time=01:45:00
/system scheduler add interval=1d name="ReplaceBlacklistTorNodesV6" on-event="/system script run ReplaceBlacklistTorNodesV6" start-date=jan/01/1970 start-time=01:55:00

# Fetch and install address list
:put ">>> Fetching list ..."
/system script run FetchBlacklistTorNodesV6;
:delay 5;
:put ">>> Installing entries ..."
/system script run ReplaceBlacklistTorNodesV6;
